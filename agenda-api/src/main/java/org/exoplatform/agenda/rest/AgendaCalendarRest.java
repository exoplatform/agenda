...
import static org.exoplatform.agenda.rest.RestUtils.DEFAULT_LIMIT;
...
public class AgendaCalendarRest {
  ...
  @RolesAllowed("users")
  @ApiOperation(value = "Retrieves the list of calendars available for currently authenticated user", httpMethod = "GET", response = Response.class, produces = "application/json")
  @ApiResponses(value = {
      @ApiResponse(code = HTTPStatus.OK, message = "Request fulfilled"),
      @ApiResponse(code = HTTPStatus.UNAUTHORIZED, message = "Unauthorized operation"),
      @ApiResponse(code = HTTPStatus.INTERNAL_ERROR, message = "Internal server error"),
  })
  public Response list(@ApiParam(value = "Search keyword to use to search for calendars", required = false) @QueryParam("q") String query,
    ...
    try {
      ...
      List<Calendar> calendars = agendaCalendarService.getCalendars(query, offset, limit, currentUser);
      ...
      if (returnSize && !calendars.isEmpty()) {
        ...
        if (returnedCalendarSize >= limit) {
          int calendarCount = agendaCalendarService.countCalendars(query, currentUser);
  ...
  @Produces(MediaType.APPLICATION_JSON)
  @RolesAllowed("users")
  @ApiOperation(value = "Retrieves the list of calendars available for an owner of type user or space, identitifed by its identity technical identifier", httpMethod = "GET", response = Response.class, produces = "application/json")
  @ApiResponses(value = {
      @ApiResponse(code = HTTPStatus.OK, message = "Request fulfilled"),
      @ApiResponse(code = HTTPStatus.UNAUTHORIZED, message = "Unauthorized operation"),
      @ApiResponse(code = HTTPStatus.INTERNAL_ERROR, message = "Internal server error"),
  })
  public Response listByOwner(@ApiParam(value = "Identity technical identifier", required = true) @PathParam("ownerId") long ownerId,
    ...
    String currentUser = RestUtils.getCurrentUser();
    try {
      List<Calendar> calendars = agendaCalendarService.getCalendarsByOwner(ownerId, query, offset, limit, currentUser);
      CalendarList calendarList = new CalendarList();
      calendarList.setCalendars(calendars);
      if (returnSize && !calendars.isEmpty()) {
        int returnedCalendarSize = calendarList.getCalendars().size();
  ...
  public Response getCalendarById(@ApiParam(value = "Calendar technical identifier", required = true) @PathParam("calendarId") long calendarId) {
    ...
    try {
      ...
      LOG.warn("Error retrieving calendars with id '{}'", calendarId, e);
      return Response.serverError().entity(e.getMessage()).build();
    }
  }
