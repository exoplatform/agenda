  ...
  <div class="d-flex align-center position-relative width-fit-content">
    <template v-for="(date, i) in dates">
      <v-sheet
        ...
        :class="i === 0 ? 'primary white--text rounded-tl' : 'background-grey-primary rounded-tr'">
        <span class="text-font-size font-weight-bold">{{ date.day }}</span>
        <span class="caption text-capitalize">{{ date.month }}</span>
      </v-sheet>
      <v-icon
