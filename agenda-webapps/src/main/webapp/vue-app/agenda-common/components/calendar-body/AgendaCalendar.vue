...
export default {
  ...
  computed: {
    ...
    canCreateEvent() {
      return (this.$root?.timelineSettings && this.$root.timelineSettings.agendaSource !== 'selectedSpaces') || (this.calendars?.length && ( !this.calendars[0]  || this.calendars.some(c => c?.acl?.canCreate)));
    },
  },
  watch: {
    mouseIsPressed() {
