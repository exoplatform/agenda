  ...
  <div class="event-details-body overflow-auto flex-grow-1 d-flex flex-column flex-md-row pa-4 mt-5">
    <div class="flex-grow-1 flex-shrink-0 event-details-body-left " :class="{ 'd-flex' : !$root.isMobile }">
      <div class="full-width" :class="{'mx-auto' : $root.isMobile}">
        ...
        <!--
          EXO-90373: an event a space subscribes to reads as the space's; this
          one line says which calendar it comes from, read-only.
        -->
        <div v-if="isSpaceSubscription" class="event-space-subscription align-center d-flex pb-5">
          <v-icon size="20" class="icon-default-color pe-5">fas fa-rss</v-icon>
          <span class="align-self-center text-break text-light-color">{{ spaceSubscriptionLabel }}</span>
        </div>
...
export default {
  ...
  /*  bodyElement() {
    ...
    /**
     * @returns {String} the line naming the calendar a space's event comes from
     */
    spaceSubscriptionLabel() {
      return this.$t('agenda.calendarSubscription.fromSpaceFeed', {0: this.event.calendar.name || ''});
    },
    /**
