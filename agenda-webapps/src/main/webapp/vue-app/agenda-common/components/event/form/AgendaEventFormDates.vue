...
export default {
  ...
  methods: {
    scrollToEvent(event) {
      const dateOption = event.dateOptions && event.dateOptions.length && event.dateOptions[0] || event;
      if (dateOption && dateOption.startDate) {
        const dateTime = this.$agendaUtils.toDate(dateOption.startDate);
        this.scrollToTimeTop = this.$refs.calendar.timeToY({
          hour: dateTime.getHours(),
          minute: dateTime.getMinutes(),
        });
        this.dayToDisplay = dateOption.startDate ? this.$agendaUtils.toDate(dateOption.startDate).getTime(): this.$agendaUtils.toDate(dateOption.start).getTime();
