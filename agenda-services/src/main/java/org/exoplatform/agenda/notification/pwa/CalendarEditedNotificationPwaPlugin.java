...
import static org.exoplatform.agenda.util.NotificationUtils.AGENDA_CALENDAR_EDITED_NOTIFICATION_PLUGIN;
...
/**
 * The installed-app push of "changes in your calendar" (EXO-90378): the
 * colleague's name in the title, the calendar's in the body, the agenda as the
 * link.
 */
public class CalendarEditedNotificationPwaPlugin implements PwaNotificationPlugin {

  private static final String         TITLE_LABEL_KEY = "pwa.notification.CalendarEditedNotificationPwaPlugin.title";
   ...
   */
  public CalendarEditedNotificationPwaPlugin(ResourceBundleService resourceBundleService) {
  ...
  @Override
  public String getId() {
    return AGENDA_CALENDAR_EDITED_NOTIFICATION_PLUGIN;
  }
  ...
  public PwaNotificationMessage process(NotificationInfo notification, LocaleConfig localeConfig) {
    ...
    String modifierName = StringUtils.defaultString(notification.getValueOwnerParameter(STORED_PARAMETER_MODIFIER_NAME));
    String title = resourceBundleService.getSharedString(TITLE_LABEL_KEY, localeConfig.getLocale());
    notificationMessage.setTitle(StringUtils.defaultString(title, TITLE_LABEL_KEY).replace("{0}", modifierName));
