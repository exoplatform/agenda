...
import static org.exoplatform.agenda.util.NotificationUtils.buildTemplateCalendarEditedParameters;
...
/**
 * Renders the "a colleague changed something in your calendar" notification
 * (EXO-90378), for the mail, on-site and push channels alike: everything the
 * template needs was stored on the notification when it was made, so nothing
 * is read again here — a deleted event has nothing left to read, and the three
 * kinds of change must read alike.
 */
public class CalendarEditedTemplateBuilder extends AbstractTemplateBuilder {

  private static final Log       LOG = ExoLogger.getLogger(CalendarEditedTemplateBuilder.class);
   ...
   *          does
   */
  public CalendarEditedTemplateBuilder(TemplateProvider templateProvider,
  ...
  protected MessageInfo makeMessage(NotificationContext ctx) {
    ...
    try {
      TemplateContext templateContext = buildTemplateCalendarEditedParameters(templateProvider, notification);
