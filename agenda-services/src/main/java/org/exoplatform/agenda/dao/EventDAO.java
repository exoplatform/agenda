...
public class EventDAO extends GenericDAOJPAImpl<EventEntity, Long> {
  ...
  public List<Long> getEventIds(Date startDate,
                                ...
                                int limit) {
    return getEventIds(startDate, endDate, ownerIds, attendeeIds, responseTypes, excludedCalendarIds, null, limit);
  }

  /**
   * Retrieves the identifiers of the events matching a period and the given
   * owner, attendee and response criteria, plus the events of some calendars
   * whatever those criteria say (EXO-90357).
   * <p>
   * One statement. The owner and attendee criteria form one conjunction, the
   * listed calendars are <em>or</em>-ed with it: an event is kept when it
   * matches the criteria, or when its calendar is listed. The attendee join
   * becomes an outer one when calendars are listed, since an event of a
   * shared calendar has no attendee row for the reader; {@code DISTINCT}
   * keeps an event with several attendee rows once. The exclusion still
   ...
   *          {@code attendeeIds}
   * @param excludedCalendarIds technical identifiers of the calendars whose
   *          events must be left out, whatever else keeps them
   * @param calendarIds technical identifiers of the calendars whose events
   *          are kept whatever the owner and attendee criteria say; empty or
   *          {@code null} for none
   * @param limit maximum number of identifiers to return, 0 for no limit
   * @return {@link List} of matching event technical identifiers
   */
  public List<Long> getEventIds(Date startDate, // NOSONAR
                                Date endDate,
                                List<Long> ownerIds,
                                List<Long> attendeeIds,
                                List<EventAttendeeResponse> responseTypes,
    ...
    boolean includeCalendars = calendarIds != null && !calendarIds.isEmpty();
    boolean filterResponses = filterAttendees && responseTypes != null && !responseTypes.isEmpty();
    ...
    if (filterAttendees) {
      jpql.append(includeCalendars ? " LEFT JOIN ev.attendees att" : " INNER JOIN ev.attendees att");
    }
    ...
    if (filterOwners) {
      criteria.add("cal.ownerId IN (:ownerIds)");
    }
    if (filterAttendees) {
      criteria.add("att.identityId IN (:attendeeIds)");
      if (filterResponses) {
        criteria.add("att.response IN (:responseTypes)");
      }
    }
    if (!criteria.isEmpty() && includeCalendars) {
      jpql.append(" AND ((").append(String.join(" AND ", criteria)).append(") OR cal.id IN (:calendarIds))");
    } else if (!criteria.isEmpty()) {
      jpql.append(" AND ").append(String.join(" AND ", criteria));
    } else if (includeCalendars) {
      jpql.append(" AND cal.id IN (:calendarIds)");
    }
    if (excludeCalendars) {
    ...
    if (includeCalendars) {
      query.setParameter("calendarIds", calendarIds);
    }
    if (filterAttendees) {
